class ClassificationComparer
  ...
  def compare
    ...
    # compare attendees
    %w[about agenda keywords attendees decisions is_agenda has_attendees
       has_decisions].each do |key|
      compare_jsons(key, classification_1.output, classification_2.output)
    end
  end

  def find_classification(model)
    classification = DocumentClassification.where(model:, document:).first
    classification = Integrations::ClassifyDocumentWorker.new.perform(document.id, model) if classification.nil?
    classification
  end
